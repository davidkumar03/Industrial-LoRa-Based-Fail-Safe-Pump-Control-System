...
/* USER CODE BEGIN PFP */
void Update_All_Pumps(void);
uint8_t Compute_CRC8(uint8_t*, uint16_t);
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Software CRC-8 (Dallas/Maxim)
...
void Update_All_Pumps()
{
       // Command Pump 1
       TxBuffer[0] = 0x01;
       TxBuffer[1] = 50;
       TxBuffer[2] = Compute_CRC8(TxBuffer, 2);
       LoRa_transmit(&myLoRa, TxBuffer, 3, 100);
       HAL_Delay(500); // Wait for air to clear
